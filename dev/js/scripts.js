...
app.personalSkills = function () {
  ...
  if(skill.length) {
    skill
      ...
      .each(function() {
        var sefl = $(this);
        var svg = '<svg viewBox="-10 -10 220 220"><circle cx="0" cy="0" fill="#26262d" r="100" stroke="#eecd10" stroke-width="20" transform="translate(100, 100)"></circle></svg>';
        
        sefl.append(svg, svg).find('>svg').last().find('>circle').attr('stroke-dashoffset', -Math.round((parseInt(sefl.data('percent').replace('%', ''))*629)/100));
      });
  }
